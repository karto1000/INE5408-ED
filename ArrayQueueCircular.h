...
class ArrayQueue {
 ...
 private:
    ...
    int _ini, _fim;  /// varaibles to indicate the start and the end
...
T ArrayQueue<T>::dequeue() {
	...
	if (empty()) {
		...
		data = contents[_ini];
		_ini = (_ini + 1) % max_size_;
		size_ -= 1;
	}
	return data;
}
/**
 * Gives the last element from my queue
 */
template <typename T>
T& ArrayQueue<T>::back() {
	if(empty()) {
